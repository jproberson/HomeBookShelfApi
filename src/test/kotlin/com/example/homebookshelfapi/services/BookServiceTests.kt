...
class BookServiceTest {
  ...
  @MockK private lateinit var googleApiService: GoogleApiService

  @MockK private lateinit var userBooksService: UserBooksService

  @MockK private lateinit var userRepository: UserRepository

  @InjectMockKs private lateinit var bookService: BookServiceImpl
  ...
  fun addBook_ShouldReturnException_WhenBookExists() {
    every { bookRepository.findByIsbn(book.isbn) } returns Optional.of(book)
    assertThrows(IllegalArgumentException::class.java) { bookService.addBook(book) }
  }
  ...
  fun deleteBook_ShouldReturnTrueWhenBookExistsForUser() {
    ...
    val isDeleted = bookService.deleteBook(book.id, user.username)
  ...
  fun deleteBook_ShouldReturnFalseWhenBookNotFound() {
    ...
    verify(exactly = 0) { userBooksService.deleteBookForUser(user.username, book.id) }
  }
}
